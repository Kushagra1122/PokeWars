...
const { verifyBlockchainTransaction, getTransactionReceipt } = require('../utils/web3Validation')
...
// body: { pokemonId, price, transactionHash }
// protected route - create a marketplace listing
exports.listForSale = async (req, res) => {
  try {
    const userId = req.user?.id
    const { pokemonId, price, transactionHash } = req.body
    ...
    // Verify blockchain transaction (optional - can be checked later)
    if (transactionHash) {
    ...
    res.json({ listing: populatedListing, transactionHash })
  } catch (err) {
...
exports.buyFromListing = async (req, res) => {
  try {
    ...
    listing.transactionHash = transactionHash || null
    ...
    res.json({ user: updatedBuyer, listing: listing._id, transactionHash, message: 'Purchase successful!' })
  } catch (err) {
...
exports.upgradePokemonLevel = async (req, res) => {
  try {
    ...
    res.json({ user: updatedUser, transactionHash, message: 'Level upgraded successfully!' })
  } catch (err) {
