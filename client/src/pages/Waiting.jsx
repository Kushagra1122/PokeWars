...
const Waiting = () => {
      ...
      <div className="w-full max-w-5xl bg-blue-950/90 border border-yellow-400 rounded-3xl shadow-2xl p-6 backdrop-blur-md">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="flex flex-col gap-6">
            <PlayersList lobby={lobby} isOwner={isOwner} currentUserId={user.id} />
            <ChatBox
              messages={messages}
              setMessage={(msg) =>
                socket.emit("sendMessage", { code, message: `${user.name}: ${msg}` })
              }
            />
          </div>
  ...
  onStartGame={() => {
    if (lobby.players.length < 2) return setError("Need at least 2 players");
    const required = ["gameTime", "map", "gameType"];
    const missing = required.filter((k) => !gameSettings[k]);
    if (missing.length) return setError(`Set all settings: ${missing.join(", ")}`);
    if (gameSettings.gameType === "rated" && (!gameSettings.stake || gameSettings.stake <= 0))
