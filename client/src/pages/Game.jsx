...
const Game = () => {
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-blue-900 via-purple-800 to-blue-900 text-yellow-400 overflow-hidden flex flex-col">
      {/* Header */}
      <div className="text-2xl font-bold p-4 text-center z-20 flex-shrink-0">
        PokiWars (
        {gameState?.settings?.gameType === "friendly"
          ? "Friendly"
          : `Ranked${
              gameState?.settings?.stake
                ? ` - ${gameState.settings.stake} Poki`
                : ""
            }`}
        )
      </div>
