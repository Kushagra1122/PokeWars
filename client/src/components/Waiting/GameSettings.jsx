  ...
  return (
    <div className="bg-yellow-400/10 border border-yellow-400 rounded-xl p-6">
      ...
      {gameSettings.gameType === "rated" && (
        <>
          <label className="block text-yellow-200 mb-2">Stake</label>
          <input
            type="number"
            min="0"
            value={gameSettings.stake || ""}
            onChange={(e) => handleUpdate("stake", parseInt(e.target.value) || 0)}
            disabled={!isOwner}
            className="w-full mb-4 p-2 rounded-lg bg-yellow-400/20 border border-yellow-400 text-blue-900"
          />
        </>
      ...
      <div className="flex gap-4">
        ...
        {isOwner && (
          <button
            onClick={onStartGame}
            className="flex-1 py-3 bg-green-500 text-white font-bold rounded-xl hover:bg-green-400"
          >
            Start
          </button>
        )}
